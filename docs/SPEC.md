...
1. **Топологія production-середовища**  
   ...
   **Рішення:** Клієнти Grafana, DSpace, Koha та Matomo працюють на одному Docker-хості в єдиній мережі Docker Swarm. Клієнт Moodle працює на іншій віртуальній машині (ВМ) під управлінням спільного гіпервізора. TLS є обов'язковим для Moodle і рекомендованим для інших клієнтів.
...
3. **Поштова скринька відправника**  
   ...
   **Рішення:** На MVP-стадії єдиним дозволеним відправником є `noreply@ldubgd.edu.ua`. На подальших етапах для кожного сервісу буде створено окрему поштову скриньку для відправки.
...
5. **Сумісність SMTP2Graph із безпечним передаванням секретів**  
   ...
   **Рішення:** Створюється тимчасова runtime-конфігурація без постійного зберігання plaintext-секретів на диску. Деплой виконується через GitHub Actions із використанням Ansible Vault, Docker Secrets та SOPS з age-шифруванням. У репозиторії буде збережено два зашифрованих файли конфігурації: `env.dev.enc` та `env.prod.enc`. Як шаблони використовуються скрипт оркестрації `scripts/deploy-orchestrator-swarm.sh` та CI/CD pipeline `.github/workflows/main.yml`.
...
1. Очікувана звичайна та пікова інтенсивність пошти, особливо Moodle digest і масові сповіщення.  
   **Рішення:** Звичайне навантаження становить менше 10 повідомлень за хвилину.
...
7. Чи прийнятна ліцензія SMTP2Graph GPL-3.0.  
   **Рішення:** Внутрішнє використання допускається після ліцензійного перегляду.
...
- Queue і failed directories доступні лише service identity та operator/root.
...
- Security/audit metadata: 90 днів.
...
- failed-message evidence лише за operational need і в зашифрованому вигляді.
...
- SOPS + age може зберігати encrypted secret source у Git лише за policy approval.
- Age private identity не зберігається в тому самому repository та не доступна untrusted CI.
- Rotation використовує versioned secret names, оскільки Docker Secrets immutable.
...
- **Призначення:** threat model, data classification, permission proof і exceptions.
...
7. `ADR-0007-single-instance-with-active-passive-recovery.md`
...
- license approval;
- pinned release;
- proof secret-file compatibility;
- non-root/read-only compatibility;
- MIME/BCC/attachments tests;
- queue durability та acknowledgement semantics;
- image scan і SBOM review.
